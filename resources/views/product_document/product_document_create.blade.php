<form id="add_form" class="form" method="POST" action="{{ route('product-doc.store') }}">
    @csrf
    <input hidden name="type" value="{{$type}}"/>
    <input hidden name="product_id" value="{{$productId}}"/>
    <div class="py-10 px-7 px-lg-10">
        <div class="row g-5">
            <div class="col-12">
                <label class="required fs-6 fw-bold mb-2">{{ __('provider.Attach File') }}</label>
                <input type="file" name="documents" class="form-control form-control-solid">
            </div>
        </div>
    </div>
