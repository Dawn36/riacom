<form id="add_form" class="form" method="POST" action="{{ route('provider.update',$provider->id) }}">
    ...
    <div class="scroll-y py-10 px-7 px-lg-10" id="modal_large" data-kt-scroll="true"
        ...
        <div class="fv-row mb-5">
            ...
            <input type="text" class="form-control form-control-solid" value="{{ $provider->name }}" placeholder="{{ __('provider.Enter Full name') }}"
                name="name" />
        </div>
        <div class="fv-row mb-5">
            <label class="fs-6 fw-semibold mb-2">{{ __('provider.Email') }}</label>
            <input type="text" class="form-control form-control-solid" value="{{ $provider->email }}" placeholder="{{ __('provider.Enter Email') }}" name="email" />
        </div>
        <div class="fv-row mb-5">
            <label class="fs-6 fw-semibold mb-2">{{ __('provider.Phone') }}</label>
            <input type="text" class="form-control form-control-solid" value="{{ $provider->phone }}" placeholder="{{ __('provider.Enter phone number') }}"
                name="phone" />
        </div>
        <div class="fv-row mb-5">
            ...
            <select name="users[]" class="form-select form-select-solid js-example-basic-single" data-dropdown-parent="#modal_large" data-control="select2"
                data-close-on-select="true" data-placeholder="{{ __('provider.Assign Users') }}" data-allow-clear="true" multiple>
                <option></option>
        ...
        <div class="fv-row mb-5">
            ...
            <input type="text" class="form-control form-control-solid kt_tagify_1"
                placeholder="{{ __('provider.Enter Type of Service') }}" value="{{ $provider->type_of_service }}" name="type_of_service" />
        </div>
    </div>
    <div class="modal-footer flex-center">
        ...
        <button id="submitbutton" type="button" onclick="addUpdateData('add_form','modal_large','yes')" class="btn btn-lg btn-primary">
            <label class="indicator-label">{{ __('provider.Update') }}</label>
